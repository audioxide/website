...
const toTitleCase = (str: string, divisor = ' ') =>
  str
    .split(divisor)
...
const albumCoverAlt = (review: Review) =>
  `Album artwork of '${review.metadata.album}' by ${review.metadata.artist}`
...
const injectRichMediaComponentAssets = (
  metadata: MetaInfo,
  ...
  components.scripts.forEach((customTag) =>
    scripts.push({
      hid: `${customTag}-js`,
