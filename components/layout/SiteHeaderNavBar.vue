    ...
    <ul class="site-nav__listing">
      ...
      >
        <nuxt-link
          ...
          >{{ item.link.text }}</nuxt-link
        >
        <ul
          v-if="!collapsed && 'children' in item"
          class="site-nav__listing--sub"
        >
          <li
...
.site-nav__logo {
  ...
  right: 0.5em;
  padding-right: calc(
    #{$site-nav__signet-size--small} + #{$site-nav__signet-gutter--small}
  );
}
...
.site-nav__logo-text {
  ...
  &:after {
    ...
    top: calc(
      (#{$site-nav__signet-size--small} - #{$site-nav__bar-font-size--small}) /
        2 * -1
    );
  }
}
