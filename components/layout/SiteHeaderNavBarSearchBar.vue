<template>
  <div class="search-bar" :class="{ active: isActive }">
    ...
    <ul v-if="isActive && isEmpty" class="search-results">
      <li>
        <ul class="listing">
    ...
    <ul v-else-if="isActive" class="search-results">
      <li v-for="(listing, key) in results" :key="key">
        <span class="heading">{{ key }}</span>
        <ul class="listing" :class="{ pills: key === 'tags' }">
          <li v-for="(item, listingKey) in listing" :key="listingKey">
            <nuxt-link tabindex="0" :to="item.route">{{
              item.title
            }}</nuxt-link>
          </li>
        </ul>
