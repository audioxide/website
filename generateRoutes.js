...
const getData = (route) =>
  fetch(`${process.env.EXT_API_URL}/${route}.json`).then((r) => r.json())
...
module.exports = async () => {
  ...
  await Promise.all(
    types.postTypes.map(
      (type) =>
        routes.push(`/${type}/`) &&
        getData(type).then((posts) =>
          posts.forEach(({ metadata: { type, slug } }) =>
            routes.push(`/${type}/${slug}/`)
          )
        )
    )
  )
  await fs.promises.writeFile(cacheFilePath, JSON.stringify(routes), {
    encoding: 'utf-8'
  })
  return routes
}
