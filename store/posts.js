...
const mergePost = (postObject, lookup, newPost, sort = false) => {
  ...
  if (path in lookup) {
    Object.keys(newPost).forEach((key) =>
      Vue.set(lookup[path], key, newPost[key])
    )
    return false
  }
...
export const getters = {
  byIndex: ({ posts }) =>
    Object.values(posts).reduce((acc, grouping) => {
  ...
  pathLookup: (_, { byIndex }) =>
    byIndex.reduce((acc, post) => {
...
export const mutations = {
  setPostsObject(state, posts) {
    Object.values(posts).forEach((postArr) =>
      mergePosts(state.posts, this.getters[getterId], postArr)
    )
  },
